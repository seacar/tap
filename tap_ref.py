...
The published vectors exercise:
  ...
  * authority binding — validity window and     [TAP-EVT-AUTHORIZATION,
    effect-match labeling (PROVISIONAL: no        TAP-AUTHORITY-VALIDITY,
    Verifier/SDK/Service-Profile enforces it yet)  TAP-AUTHORITY-EFFECT, §9.2]
...
# ---------- authority binding (§9.2, provisional) ----------
# Unlike revocation ([TAP-KEY-REVOCATION]) and replay defense ([TAP-REPLAY]), the
# checks below are stateless — no registry lookup — so they belong here as pure
# functions, the same way checkpoint reconciliation does. `[TAP-AUTHORITY-REVOKE]`
# and `[TAP-AUTHORITY-REUSE]` are stateful (they need a store of consumed/revoked
# ids) and are therefore a Verifier/Service-Profile concern, not this file's.

def check_authority_window(authorization: dict, now: int) -> None:
    """Enforce the authorization validity window [TAP-AUTHORITY-VALIDITY, §9.2],
    with the same ±60s skew allowance as Passport freshness
    [TAP-PASSPORT-VALIDATE, §5.3]. Raises ValueError outside the window."""
    nbf, exp = authorization["nbf"], authorization["exp"]
    if not (nbf - 60 <= now < exp + 60):
        raise ValueError(
            f"authorization outside validity window: nbf={nbf} exp={exp} now={now}"
        )

def authority_effect_label(authorization: dict | None, result: dict) -> str | None:
    """Label an Event's authority-binding outcome [TAP-AUTHORITY-EFFECT, §9.2] by
    comparing `result.effect_digest` against `authorization.target_state_digest`.
    Returns None when no `authorization` is present (the label does not apply);
    otherwise one of "authorized_match" / "authorized_mismatch" /
    "unverified_authority". Reported ALONGSIDE [TAP-ASSURANCE, §7], never in place
    of it — the two check different things."""
    if authorization is None:
        return None
    effect = result.get("effect_digest")
    if effect is None:
        return "unverified_authority"
    return ("authorized_match" if effect == authorization["target_state_digest"]
            else "authorized_mismatch")
...
if __name__ == "__main__":
    ...
    # ----- authority-binding event [TAP-EVT-AUTHORIZATION, §9.2] (provisional) -----
    # A server-attested tool_call bound to a pre-declared expected effect. This is
    # the case a compromised-but-authentic Signer cannot fake: even a fully valid,
    # independently server-attested Event can now be checked against the specific
    # outcome someone actually approved, not merely against a class-of-action rule.
    EVT_AUTH = "evt_01JABCDEFGHJKMNPQRSTVWXYZ4"
    auth_intent = "Issue a $50.00 refund to the account holder for ticket #8842"
    auth_args = {"ticket": "8842", "refund_cents": 5000}
    # The authority state this approval was granted under — same digest
    # construction as policy_version (§9.1): sha256 over JCS-canonical bytes.
    authority_state = {"policy": "refund-v3", "rules": ["max_refund_cents:10000"]}
    expected_state = {"ticket": "8842", "status": "refunded", "refund_cents": 5000}
    mismatched_state = {"ticket": "8842", "status": "refunded", "refund_cents": 7500}
    authorization = {
        "authz_id": "auz_01JABCDEF5555555555555555",
        "authority_state_version": json_digest(authority_state),
        "target_state_digest": json_digest(expected_state),
        "nbf": 1767225600,                 # matches the Passport's iat, same record
        "exp": 1767229200,                 # matches the Passport's exp
        "issuer_kid": KID,                  # here the same key; MAY be a distinct
                                             # human/admin key in a real deployment
    }
    auth_body = {
        "v": "tap/0.1",
        "event_id": EVT_AUTH,
        ...
        "cid": cid,
        "seq": 11,
        "ts": "2026-01-01T00:03:13.900Z",
        "action": {
            "kind": "tool_call",
            "intent_digest": text_digest(auth_intent),
            "tool": "issue_refund",
            "scope_used": "call:tool",
            "args_digest": json_digest(auth_args),
        },
        "authorization": authorization,
        "result": {"status": "success", "code": "OK", "latency_ms": 91, "error": None,
                   "effect_digest": json_digest(expected_state)},
        "attestor": "server",
        "action_ref": "act_01JABCDEF6666666666666666",
        "kid": KID,
    }
    auth_event = sign_event(sk, auth_body)
    auth_annex = {
        "event_id": EVT_AUTH,
        "intent": auth_intent,
        "args_preview": auth_args,
    }
    ...
    for ev in (tool_event, dec_event, ckpt_event, canon_event, auth_event):
    ...
    # authorization itself is correctly ABSENT on every event that has no bound
    # approval — only auth_event carries it
    for ev in (tool_event, dec_event, ckpt_event, canon_event):
        assert "authorization" not in ev, "authorization must be omitted when absent, not null"
    assert "effect_digest" not in tool_event["result"], \
        "effect_digest must be omitted when unmeasured, not null"
    ...
    # authority binding: validity window, single-source-of-truth digest reuse with
    # policy_version's construction, and the match/mismatch/unverified labels
    # [TAP-AUTHORITY-VALIDITY, TAP-AUTHORITY-EFFECT, §9.2]
    check_authority_window(authorization, now=1767225700)  # mid-window: must not raise
    for bad_now in (authorization["nbf"] - 61, authorization["exp"] + 61):
        try:
            check_authority_window(authorization, now=bad_now)
            raise SystemExit(f"FAIL: now={bad_now} outside the window was accepted")
        except ValueError:
            pass
    assert authority_effect_label(None, auth_body["result"]) is None
    assert authority_effect_label(authorization, auth_body["result"]) == "authorized_match"
    assert authority_effect_label(authorization, {"effect_digest": json_digest(mismatched_state)}) \
        == "authorized_mismatch"
    assert authority_effect_label(authorization, {}) == "unverified_authority"
    ...
    vectors = {
        ...
        "authorization": {
            "_about": (
                "Authority binding (§9.2, PROVISIONAL — schema and rules are "
                "normative, but no Verifier, SDK, or Service Profile implements "
                "enforcement yet). A server-attested tool_call bound to a "
                "pre-declared expected effect, plus fixtures for the validity-"
                "window and effect-match checks other implementations can test "
                "against without needing their own fixed seed."
            ),
            "canonical_signing_input_sha256": hashlib.sha256(signing_input(auth_event)).hexdigest(),
            "signed_event": auth_event,
            "annex": auth_annex,
            "authority_state_version_construction": {
                "authority_state": authority_state,
                "authority_state_version": authorization["authority_state_version"],
                "_note": "same construction as policy_version (§9.1): "
                         "sha256(JCS(canonical_state))",
            },
            "validity_window": {
                "nbf": authorization["nbf"],
                "exp": authorization["exp"],
                "accepted_now": 1767225700,
                "rejected_now_not_yet_valid": authorization["nbf"] - 61,
                "rejected_now_expired": authorization["exp"] + 61,
                "_note": "±60s skew allowance, same inequality as "
                         "[TAP-PASSPORT-VALIDATE, §5.3]: nbf-60 <= now < exp+60",
            },
            "effect_check": {
                "target_state_digest": authorization["target_state_digest"],
                "authorized_match_effect_digest": auth_body["result"]["effect_digest"],
                "authorized_mismatch_effect_digest": json_digest(mismatched_state),
                "_note": "compare result.effect_digest to authorization."
                         "target_state_digest; absent effect_digest -> "
    ...
    print("\nAll self-checks passed: passport ✓  tool_call ✓  decision ✓  checkpoint ✓  "
          "canonicalization ✓  authorization ✓  omit-nulls ✓  guard ✓  suite ✓  "
