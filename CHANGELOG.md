...
A correctness pass across both SDKs, found by a new cross-language regression
and envelope-parity harness (`sdk/*/test/regressions.test.ts`,
`sdk/python/tests/test_regressions.py`, `scripts/check-envelope-parity.py`) —
none of these were wired into CI yet, so each bug below shipped invisibly
behind a fully green test suite. No wire-version change; `test-vectors.json`
and `tap_ref.py` are unaffected.

### Fixed

- **Passport verification used to strip under `python -O`**: `verify_passport`'s
  `typ`/`kid`/freshness checks were `assert` statements, which Python removes
  entirely when the interpreter runs with `-O` — a flag any production image
  may set. They now raise a new typed `InvalidRecord` (base) / `PassportExpired`
  (the freshness case specifically), which no interpreter flag can strip.
  Mirrored in TypeScript (`PassportExpired` on `tap.ts`); Go raises plain
  errors already and was unaffected (`go build` has no `-O`-equivalent).
- **JOSE `alg:none` was never checked** in `verifyPassport`/`verify_passport`/
  `VerifyPassport` (Python, TypeScript, Go): suite dispatch reads the *key's*
  declared algorithm, never the token header's, so a header claiming
  `alg:none` alongside a perfectly valid Ed25519 signature passed silently
  [TAP-SIG-ALG]. All three now reject a header `alg` other than `EdDSA`.
- **A malformed `authorization` block crashed the Verifier** instead of being
  labeled: `authority_effect_label` / `authorityEffectLabel` indexed into the
  block without checking its shape, so a signed event carrying
  `"authorization": {}` (or a string, list, or number) raised `KeyError` /
  `TypeError` out of `evaluate_event` — one poisoned event could take down an
  entire audit report. Both now return the existing `"malformed_authority"`
  label instead of raising. The JS `evaluateEvent` also gained the
  `authorization` / `authority_effect` fields on its result — previously
  present in Python's `evaluate_event` but absent from the TypeScript port
  entirely.
- **`check_authority_window` raised raw `KeyError`/`TypeError`** on a malformed
  block instead of failing closed with a named error; it now raises
  `AuthorityMalformed` (Python), matching the TypeScript SDK's existing
  behavior.
- **`result.latency_ms: null` vs. omitted key**: the Python SDK's `checkpoint`,
  `decision`, and `model_response` events always signed an explicit
  `latency_ms: null`, while the TypeScript SDK omitted the key entirely for
  the same event kinds — different signed bytes for logically identical
  events, which is exactly what [TAP-EVT-OMIT] exists to prevent. Python now
  omits the key when no latency was measured, matching TypeScript.
- **`record_output` used a different `tool` string per SDK**: Python signed
  `"agent.run"`, TypeScript signed `"agent.record"`. Python now matches
  TypeScript's `"agent.record"` (consistent with both SDKs' own
  `agent:record` default scope).
- **TypeScript SDK failed to build**: `instrumentMcp` passed `latencyMs` to
  `TAPClient.signToolCall`, whose type didn't declare that field, so `npm run
  build` errored (`tsc` — TS2353) and the measured MCP-call latency was
  silently dropped even in code that did compile against an older build.
  `signToolCall` now accepts and forwards `latencyMs`, matching `traceTool`.

### Known gap surfaced, not fixed in this release

- **Two-sided assurance does not check signing-key independence.**
  `assuranceLevel` / `assurance_level` (both SDKs) label a `server` leg
  "two-sided" purely on content agreement (`cid`, `action.tool`,
  `action.kind`, result consistency) — they never check that the `agent` and
  `server` legs were signed by *different* keys. A Signer can therefore mint
  a self-consistent fake server leg with its own key and have it verified as
  independently-attested [TAP-ASSURANCE-KEY]. This is the most significant
  finding of the pass and is tracked, not patched here, because closing it
  needs a trust-anchor API decision (a `is_server_key` callback, already
  drafted in the new regression tests) reviewed across both SDKs rather than
  a same-session fix. See `outstanding-tasks.md`.
